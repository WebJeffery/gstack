...
这是 **OpenClaw + GStack 中文文档站**，基于 VitePress 构建的静态文档网站，部署在 Azure Static Web Apps。文档内容覆盖 OpenClaw（原名 ClawdBot）的安装教程、源码剖析和 AI 框架解析，以及 **GStack** 仓库的 **源码导读与开发文档**（`docs/gstack/`，按模块分子目录，与 garrytan/gstack 顶层结构对齐）。
...
npm run gen:gstack-skills  # 根据 gstack 仓库 docs/skills.md 表格，重生成 docs/gstack/skills/*.md
```
...
docs/
├── index.md                          # 首页
├── gstack/                           # GStack：专栏首页 + 各模块子目录（repo、browse、scripts…）
│   ├── skills/                       #   每条斜杠命令一页（01-xxx.md）；源数据见 scripts/gen-gstack-skill-pages.mjs
│   ├── repo/                         #   仓库根级文档（安装、技能、开发命令、文档约定）
│   ├── browse/ …                     #   与源码目录一一对应；详见 gstack/repo/04-docs-layout.md
...
`config.mts` 中的 `listMdItems(dir)` 会**自动扫描单层目录**中的 `.md`（排除 `index.md`）生成条目。GStack 专栏在 **`gstackSidebar`** 中为每个模块注册一组侧边栏，组内为「模块概览 + `listMdItems('gstack/<模块>')`」。**新增模块**时：新建 `docs/gstack/<name>/`（含 `index.md` 与正文），并在 `gstackSidebar` 中追加一组（详见 `gstack/repo/04-docs-layout.md`）。
