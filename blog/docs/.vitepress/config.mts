...
function extractTitle(filePath: string, fallback: string): string {
  try {
    ...
    if (match) {
      // 去掉开头的编号前缀如 "00 " "01 " "12 "；去掉 H1 中的反引号（如 # `/cmd` → 侧边栏显示 /cmd）
      return match[1].replace(/^\d+\s+/, '').replace(/`/g, '').trim()
    }
  } catch {}
  // 去掉文件名中的编号前缀
...
/** GStack 专栏：按仓库模块分子目录，见 gstack/repo/04-docs-layout.md */
const gstackRepo = listMdItems('gstack/repo')
const gstackBrowse = listMdItems('gstack/browse')
const gstackDesign = listMdItems('gstack/design')
const gstackScripts = listMdItems('gstack/scripts')
const gstackTest = listMdItems('gstack/test')
const gstackExtension = listMdItems('gstack/extension')
const gstackLib = listMdItems('gstack/lib')
const gstackBin = listMdItems('gstack/bin')
const gstackSkillPipeline = listMdItems('gstack/skill-pipeline')
const gstackSkills = listMdItems('gstack/skills')

const gstackSidebar = [
  {
    text: '专栏',
    items: [{ text: '首页与模块地图', link: '/gstack/' }],
  },
  {
    text: 'skills · 斜杠命令',
    collapsed: false,
    items: [{ text: '技能索引', link: '/gstack/skills/' }, ...gstackSkills],
  },
  {
    text: '仓库根 · 开发与导览',
    collapsed: false,
    items: [{ text: '目录索引', link: '/gstack/repo/' }, ...gstackRepo],
  },
  {
    text: 'browse · 无头浏览器 CLI',
    collapsed: true,
    items: [{ text: '模块概览', link: '/gstack/browse/' }, ...gstackBrowse],
  },
  {
    text: 'design · Design CLI',
    collapsed: true,
    items: [{ text: '模块概览', link: '/gstack/design/' }, ...gstackDesign],
  },
  {
    text: 'scripts · 构建与 DX',
    collapsed: true,
    items: [{ text: '模块概览', link: '/gstack/scripts/' }, ...gstackScripts],
  },
  {
    text: 'test · 测试与 Eval',
    collapsed: true,
    items: [{ text: '模块概览', link: '/gstack/test/' }, ...gstackTest],
  },
  {
    text: 'extension · Chrome 扩展',
    collapsed: true,
    items: [{ text: '模块概览', link: '/gstack/extension/' }, ...gstackExtension],
  },
  {
    text: 'lib · 共享库',
    collapsed: true,
    items: [{ text: '模块概览', link: '/gstack/lib/' }, ...gstackLib],
  },
  {
    text: 'bin · CLI 工具',
    collapsed: true,
    items: [{ text: '模块概览', link: '/gstack/bin/' }, ...gstackBin],
  },
  {
    text: 'skill-pipeline · 模板与生成',
    collapsed: true,
    items: [{ text: '模块概览', link: '/gstack/skill-pipeline/' }, ...gstackSkillPipeline],
  },
]
  ...
  '@graph': [
    {
      ...
      '@id': `${SITE_URL}/#website`,
      url: `${SITE_URL}/`,
      name: 'OpenClaw + GStack 中文文档',
      alternateName: ['ClawdBot 文档', 'ClawdBot Docs', 'openclaw docs', 'OpenClaw 源码剖析', 'GStack 专栏', 'GStack 技能库'],
      description: 'OpenClaw 中文完整文档与 GStack AI 工程工作流技能导读：安装部署、源码剖析、Gateway 与通道；以及斜杠技能评审与发版流程。',
      inLanguage: 'zh-CN',
      potentialAction: {
        '@type': 'SearchAction',
    ...
    },
    {
      ...
      mainEntity: [
        ...
        {
          '@type': 'Question',
          name: 'GStack 是什么？和 OpenClaw 什么关系？',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'GStack 是开源的 AI 工程工作流技能库（SKILL.md），提供 /review、/qa、/ship、/browse 等斜杠命令，用于仓库内评审、测试与发版节奏。OpenClaw 是多通道 AI 智能体运行时（Gateway 与聊天通道）。二者互补：OpenClaw 侧重消息通道与网关，GStack 侧重本地开发与 PR 流程。本站 GStack 专栏路径为 /gstack/。',
          },
        },
      ],
    },
...
export default withMermaid(defineConfig({
  ...
  lang: 'zh-CN',
  title: 'OpenClaw + GStack 中文文档 | 源码剖析 · 安装教程 · AI 智能体与工作流',
  titleTemplate: ':title | OpenClaw + GStack 中文文档',
  description: 'OpenClaw 中文完整文档（276+ 篇）与 GStack AI 工程技能库导读同站：多通道智能体、Gateway、通道适配器源码剖析；GStack 斜杠工作流（评审/QA/发版）。原名 ClawdBot。',
  cleanUrls: true,
  lastUpdated: true,
  ...
  head: [
    ...
    ['meta', { property: 'og:site_name', content: 'OpenClaw + GStack 中文文档' }],
  ...
  transformHead({ page, title, description: pageDesc }) {
    ...
    const ogTitle = title || 'OpenClaw + GStack 中文文档 | 源码剖析 · 安装教程 · AI 智能体与工作流'
    const ogDesc = pageDesc || 'OpenClaw 中文完整文档（276+ 篇）与 GStack 技能库导读：多通道智能体框架 + 评审/QA/发版斜杠工作流。原名 ClawdBot。'
    return [
  ...
  themeConfig: {
    ...
    nav: [
      ...
      { text: '重点框架', link: '/beginner-openclaw-framework-focus/' },
      {
        text: 'GStack',
        items: [
          { text: '专栏首页', link: '/gstack/' },
          { text: '技能命令（/skills）', link: '/gstack/skills/' },
          { text: '仓库根 · 开发', link: '/gstack/repo/' },
          { text: 'browse', link: '/gstack/browse/' },
          { text: 'scripts', link: '/gstack/scripts/' },
          { text: 'test · Eval', link: '/gstack/test/' },
          { text: 'skill-pipeline', link: '/gstack/skill-pipeline/' },
        ],
      },
    ],
    sidebar: {
    ...
    footer: {
      message: 'OpenClaw 工程拆解 · GStack AI 工作流技能',
      copyright: 'OpenClaw + GStack Docs',
    },
    outline: {
      label: '页面目录',
      level: [2, 3],
