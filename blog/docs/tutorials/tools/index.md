...
| [canvas（画布）](/tutorials/tools/canvas) | 在 iOS/Android/macOS 节点上呈现可视化界面 | → |
| [image_generate（图像生成）](/tutorials/tools/image-generate) | 通过 AI 生成图像 | → |
| [loop-detection（循环检测）](/tutorials/tools/loop-detection) | 检测并中止 Agent 的无进展重复调用 | → |
